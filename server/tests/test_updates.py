...
RELEASES = [
    {
        "tag": "v1.1.21",
        "name": "LabWatch 1.1.21",
        "published_at": "2026-09-16T05:55:00Z",
        "html_url": "https://github.com/karpus2807/iitd_lab/releases/tag/v1.1.21",
        "notes": "Installer password prompt no longer looks stuck; alert count and clear",
        "prerelease": False,
    },
    {
        "tag": "v1.1.20",
        "name": "LabWatch 1.1.20",
        "published_at": "2026-09-16T05:45:00Z",
        "html_url": "https://github.com/karpus2807/iitd_lab/releases/tag/v1.1.20",
        "notes": "SoC vs PCIe UI, Jetson power/unified RAM, clear history/events/logs",
        "prerelease": False,
    },
    {
        "tag": "v1.1.19",
        "name": "LabWatch 1.1.19",
        "published_at": "2026-09-16T05:35:00Z",
        "html_url": "https://github.com/karpus2807/iitd_lab/releases/tag/v1.1.19",
        "notes": "Installer no longer looks stuck after username; password is hidden on purpose",
        "prerelease": False,
    },
    {
        "tag": "v1.1.18",
...
async def test_list_last_three_builds_and_apply(client: AsyncClient, auth_headers, update_harness):
    ...
    assert idle["current"]["tag"] == "v1.1.21"
    ...
    assert body["latest"]["tag"] == "v1.1.21"
    assert body["current"]["tag"] == "v1.1.21"
    assert body["source"] == "github"
    assert body["builds"][0]["is_latest"] is True
    assert any(b["is_current"] for b in body["builds"])

    apply = await client.post("/api/admin/updates/apply", headers=auth_headers, json={"tag": "v1.1.19"})
    assert apply.status_code == 200, apply.text
    status = apply.json()["status"]
    assert status["tag"] == "v1.1.19"
