...
RELEASES = [
    ...
    },
    {
        ...
        "html_url": "https://github.com/karpus2807/iitd_lab/releases/tag/v1.1.3",
        "notes": "Fix Postgres timezone seed crash",
        "prerelease": False,
    },
]
...
def test_annotate_current_and_latest():
    ...
    older = next(b for b in builds if b["tag"] == "v1.1.3")
...
async def test_list_last_three_builds_and_apply(client: AsyncClient, auth_headers, update_harness):
    ...
    assert body["latest"]["tag"] == "v1.1.5"
    assert body["current"]["tag"] == "v1.1.5"
    assert body["builds"][0]["is_latest"] is True
    assert any(b["is_current"] for b in body["builds"])

    apply = await client.post("/api/admin/updates/apply", headers=auth_headers, json={"tag": "v1.1.4"})
    assert apply.status_code == 200, apply.text
    status = apply.json()["status"]
    assert status["tag"] == "v1.1.4"
